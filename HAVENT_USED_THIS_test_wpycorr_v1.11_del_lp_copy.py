...
import xml.etree.ElementTree as ET   # use ET for modifying the xml of the kmz to add colorbar
import osr
...
parser.add_argument('-no_display', 
                    action='store_true',
                    default=False, 
                    help='no matplotlib figures will be displayed - only text in terminal - [at least one figure shown]')
...
parser.add_argument('-offset_correction', 
                    action='store_true',
                    default=False, 
                    help='estimate offset at slow moving wcs areas and make corrected output vels (requires -wcs specified) - [no]')
...
if (args.wcs):
	if args.wcs=='grn':
		...
		wcslayer='Rignot SAR Velocity - speed'
		print 'using vel layer %s'%(wcslayer)
		print 'available layers - wcs.contents.keys():',wcs.contents.keys()		
	else:
...
if (args.nsidc_out_names):  # use nsidc name conventions and out directories
	...
	if (args.out_name_base=='temp_out'):
	    out_name_base=img1.filename[0:20]+'_'+img2.filename[0:20]+'_'+str(midpoint)
	else:
	    out_name_base=args.out_name_base

	# output directory setup - currently outputs files to a directory within the pXXX_rXXX directory in the format img1name_imge2name_midpointdate_delt (MK)

	out_parent_dir=args.imgdir+'/'+out_name_base+'_'+str(int(del_t_days))+'/'
...
# mgx,mgy=np.meshgrid(np.arange(-0.9,0.9 + (0.1*subpixel_gridstep),subpixel_gridstep),
# 				    np.arange(-0.9,0.9 + (0.1*subpixel_gridstep),subpixel_gridstep),
# 				    indexing='xy')  # sub-pixel mesh - adding a tenth of a step the the high end ensures that 1.0 is in grid
...
if (args.no_display==False) & (args.vf==True):
...
orig_int_del_i=np.zeros((r_j_2.__len__(),r_i_2.__len__()), dtype=np.float32)
orig_int_del_j=np.zeros((r_j_2.__len__(),r_i_2.__len__()), dtype=np.float32)
count_tenths=np.zeros((r_j_2.__len__(),r_i_2.__len__()), dtype=np.int16)
count_hundredths=np.zeros((r_j_2.__len__(),r_i_2.__len__()), dtype=np.int16)
...
##########  Marin you may have to modify this to work for you? ####################################
	outwcs_tif_filename=file_name_base + '_wcsget.tif'
	f=open(outwcs_tif_filename, 'wb')
	f.write(getcov.read())
	f.close()
	wcs_vel=GeoImg(outwcs_tif_filename,in_dir='.')
	...
	for j in range(min_ind_j,max_ind_j+1):
		...
		if(test_for_masked_pix>0.0):  # there are all greater than zero pixels, so out of mask (valid original image can't have pixel values less than or equal to zero...)
			...
			if use_wcs:
				...
				vmax=1.5 * wcsvel / 365.25  # wcs speeds are in m/yr... we need m/day  And, allow for 50% greater speed... plus a few pixels
				maxpixoffset = np.ceil(vmax * del_t_days / img1.pix_x_m)
				new_half_target_chip = half_source_chip + maxpixoffset + 20 # set target chip size to a few more than (1.5 * max v * interval)/pixsize
...
# 				if (np.abs(spoffi) > 0.5):
# 					peak1_loc[0]=peak1_loc[0]+np.sign(spoffi)
# 					spoffi-=np.sign(spoffi)
# 				if (np.abs(spoffj) > 0.5):
# 					peak1_loc[1]=peak1_loc[1]+np.sign(spoffj)
# 					spoffj-=np.sign(spoffj)
# 				if (np.abs(spoffi) > 0.5):
# 					peak1_loc[0]=peak1_loc[0]+np.sign(spoffi)
# 					spoffi-=np.sign(spoffi)
# 				if (np.abs(spoffj) > 0.5):
...
if (not(args.no_display) & args.vf):  # include correlation and delcorr figures
	ovdec=10.0
	fig=plt.figure()
...
if args.offset_correction:
	slow_area_zero_speed=20.0 # m/yr
	if args.wcs=="":
		...
		d_i_m=np.ma.masked_where((wcs_vel.img>slow_area_zero_speed)|(wcs_vel.img==0.0)|((del_i==0.0))|(corr_arr<0.3),del_i)
		d_j_m=np.ma.masked_where((wcs_vel.img>slow_area_zero_speed)|(wcs_vel.img==0.0)|((del_j==0.0))|(corr_arr<0.3),del_j)
		pts_d_i_m=d_i_m[d_i_m.mask==False].flatten()
		pts_d_j_m=d_j_m[d_i_m.mask==False].flatten()
		pts_wcs_m=wcs_vel.img[d_i_m.mask==False].flatten()
		if len(pts_wcs_m)<100:
					print 'not enough slow moving pixels in WCS vel to do offset correction'
					del_i_corrected=del_i
					del_j_corrected=del_j
                else:
					del_i_corrected=del_i
					del_j_corrected=del_j
					del_i_corrected[((del_i!=0) | (del_j!=0))]-=np.median(pts_d_i_m)
					del_j_corrected[((del_i!=0) | (del_j!=0))]-=np.median(pts_d_j_m)
					# pix_dist_corrected_m=np.ma.masked_where(((img_oi.img==0.0))|(img_corr.img<0.3),(15.0/32.0)*np.sqrt(oi_corrected**2.0 + oj_corrected**2.0))
					print 'found offset correction (del_i: %f del_j: %f)in slow moving areas (less than %f m/yr) using %d pixels'%(-np.median(pts_d_i_m),-np.median(pts_d_j_m),slow_area_zero_speed,pts_d_i_m.shape[0])
					if not(args.nlf):
							outsf.write('\n found offset correction (del_i: %f del_j: %f)in slow moving areas (less than %f m/yr) using %d pixels\n'%(-np.median(pts_d_i_m),-np.median(pts_d_j_m),slow_area_zero_speed,pts_d_i_m.shape[0]))
...
if (not(args.no_display) & args.vf):
	plt.figure()
...
if (not(args.no_display) & args.pixfigs):	# show pixel offsets
	fig=plt.figure()
	ax=plt.subplot(121,axisbg=(0.1,0.15,0.15))
	ax1_img=plt.imshow(img1.img_ov10[np.floor(c1_minj/ovdec):np.floor(c1_maxjp1/ovdec),np.floor(c1_mini/ovdec):np.floor(c1_maxip1/ovdec)],
						 interpolation='nearest',cmap='gray',vmin=args.gs_min,vmax=args.gs_max,extent=[com_min_x,com_max_x,com_min_y,com_max_y])
...
if not(args.no_gtif):   ###### This flag is not presently used, as GTiff forms basis for all output right now...
	...
	if not(args.only_vel_colormap):		# output full suite of float geoTiffs (default behavior) - otherwise only get only color mapped speed above
		...
		if args.debug_tifs:  # output debug tifs for subpixel tracking...
			dst_filename = file_name_base+'_count_ten.tif'
			(out_lines,out_pixels)=del_corr_arr.shape
			out_bands=1
			dst_ds = driver.Create( dst_filename, out_pixels, out_lines, out_bands, gdal.GDT_Int16 )
			print 'out image %s %s %d, %d, %d'%(dst_filename, format, out_pixels, out_lines, out_bands)
			if not(args.nlf):
				outsf.write('out image %s %s %d, %d, %d\n'%(dst_filename, format, out_pixels, out_lines, out_bands))
			dst_ds.SetGeoTransform( [ output_array_ul_corner[0], output_array_pix_x_m, 0, output_array_ul_corner[1], 0, output_array_pix_y_m ] ) # note pix_y_m typically negative
			dst_ds.SetProjection( img1.proj )
			dst_ds.GetRasterBand(1).WriteArray( (count_tenths).astype('int16') )
			dst_ds = None # done, close the dataset

			dst_filename = file_name_base+'_count_hun.tif'
			(out_lines,out_pixels)=del_corr_arr.shape
			out_bands=1
			dst_ds = driver.Create( dst_filename, out_pixels, out_lines, out_bands, gdal.GDT_Int16 )
			print 'out image %s %s %d, %d, %d'%(dst_filename, format, out_pixels, out_lines, out_bands)
			...
			dst_ds.SetGeoTransform( [ output_array_ul_corner[0], output_array_pix_x_m, 0, output_array_ul_corner[1], 0, output_array_pix_y_m ] ) # note pix_y_m typically negative
			dst_ds.SetProjection( img1.proj )
			dst_ds.GetRasterBand(1).WriteArray( (count_hundredths).astype('int16') )
			dst_ds = None # done, close the dataset
		
			dst_filename = file_name_base+'_test_orig_int_offset_i.tif'
			(out_lines,out_pixels)=del_i.shape
			out_bands=1
			dst_ds = driver.Create( dst_filename, out_pixels, out_lines, out_bands, gdal.GDT_Float32 )
			print 'out image %s %s %d, %d, %d'%(dst_filename, format, out_pixels, out_lines, out_bands)
			...
			dst_ds.SetGeoTransform( [ output_array_ul_corner[0], output_array_pix_x_m, 0, output_array_ul_corner[1], 0, output_array_pix_y_m ] ) # note pix_y_m typically negative
			dst_ds.SetProjection( img1.proj )
			dst_ds.GetRasterBand(1).WriteArray( (orig_int_del_i).astype('float32') )
			dst_ds = None # done, close the dataset
	
			dst_filename = file_name_base+'_test_orig_int_offset_j.tif'
			(out_lines,out_pixels)=del_i.shape
			out_bands=1
			dst_ds = driver.Create( dst_filename, out_pixels, out_lines, out_bands, gdal.GDT_Float32 )
			print 'out image %s %s %d, %d, %d'%(dst_filename, format, out_pixels, out_lines, out_bands)
			...
			dst_ds.SetGeoTransform( [ output_array_ul_corner[0], output_array_pix_x_m, 0, output_array_ul_corner[1], 0, output_array_pix_y_m ] ) # note pix_y_m typically negative
			dst_ds.SetProjection( img1.proj )
			dst_ds.GetRasterBand(1).WriteArray( (orig_int_del_j).astype('float32') )
			dst_ds = None # done, close the dataset
