...
for jsonFn in jsonList:
	...
	print "Processing " + transName, iterCount
	iterCount += 1
	...
	seasonalVelocityDictToJson = splitSummerWinterToJson(profileDataSwathOutliers2,summerStartEnd)
	...
	# Look at timing
	winterCorr = seasonalVelocityDict2['winterCorr']
	summerCorr = seasonalVelocityDict2['summerCorr']
	
	for i in range(0,len(winterCorr)):
		wiMidDates.append(winterCorr[i]['mid_dec_year'])
		wiStarts.append(winterCorr[i]['start_dec_year'])
		wiEnds.append(winterCorr[i]['stop_dec_year'])
		wiDts.append(winterCorr[i]['delts_days'])
#		wiDoys.append(wgt.decYearToDoy(winterCorr[i]['mid_dec_year']))
		wiDoys.append(decYearToDoy(winterCorr[i]['mid_dec_year']))

	for i in range(0,len(summerCorr)):
		suMidDates.append(summerCorr[i]['mid_dec_year'])
		suStarts.append(summerCorr[i]['start_dec_year'])
		suEnds.append(summerCorr[i]['stop_dec_year'])
		suDts.append(summerCorr[i]['delts_days'])
#		suDoys.append(wgt.decYearToDoy(summerCorr[i]['mid_dec_year']))
		suDoys.append(decYearToDoy(summerCorr[i]['mid_dec_year']))
