...
# parameters for subsetting image
xMin = 579
yMax = 739
xSpan = 500
ySpan = 380
...
vx = vxImage.ReadAsArray(xMin,yMax,xSpan,ySpan)
vy = vyImage.ReadAsArray(xMin,yMax,xSpan,ySpan)
vx[vx==-1]=np.nan
vy[vy==-1]=np.nan
...
XThin, YThin, vxThin, vyThin, vvThin = thinVectors(x,y,vx,vy,vv,3)
...
thetaThinClean = np.hypot(vxThinClean,vyThinClean)

scaleFactor = 10
#lws = np.linspace(0,2,YThin.size)
plt.subplot(111)
#plt.imshow(vv,extent=[x.min(), x.max(), y.min(), y.max()])
plt.imshow(np.rad2deg(theta),extent=[x.min(), x.max(), y.min(), y.max()],origin='upper')
plt.colorbar()
#Q = plt.quiver(XThin,YThin,vxThinClean,vyThinClean,vvThinClean,width=1e2,units='xy',angles='xy',scale=2.5e-4)
#Q = plt.quiver(XThin,YThin,vxThinClean,vyThinClean,color='white',width=1e2,units='xy',angles='xy',scale=2.5e-5)
plt.subplot(111)
Q = plt.quiver(YThin,XThin,vxThinClean,vyThinClean,np.rad2deg(thetaThinClean),units='xy',angles='xy',scale=5e-4)
plt.colorbar()
