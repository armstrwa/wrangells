...
if billyCpuToggle == 1:
	...
	demFn = '/Users/anderson/Desktop/ARMSTRONG/wrangells/dtm/alaska_yukon_merge_epsg4326.tif'
...
import pyproj
from shapely.ops import stransform
...
def getUtmZones(shapelyGeometry):
	...
	for zone in utmZones:
		...
		if zoneGeom.intersects(profGeom):
...
clines = fiona.open(wgs84centerlinesFn)
for featNum in range(0,len(clines)): # iterate over all transects
	...
	# Reproject transect into local utm crs
	utmZone = getUtmZones(shapelyLineGeom)
	utmEPSG = getEPSGforUTMzone(utmZone[0])
	project = partial(
		pyproj.transform,
		pyproj.Proj(init='epsg:4326'), # source coordinate system
		pyproj.Proj(init='epsg:'+str(utmEPSG))) # destination coordinate system
	utmLineGeom = transform(project, shapelyLineGeom)  # apply projection to get centerline profile in utm
	
	# Subset south-central alaska DEM to glacier of interest
	ulI,ulJ = dem.index(lonMin,latMax)
	lrI,lrJ = dem.index(lonMax,latMin)
	dRow = lrI - ulI
	dCol = lrJ - ulJ

	demSub = dem.read(window = ((ulI,lrI),(ulJ,lrJ)))
	dLon = 0.0014967408541356833
	dLat = -0.0014967408541356833
	
	dst_transform = ( (lonMin, dem.transform[1], 0., latMax, 0., dem.transform[5] ) ) 	
	destination = np.zeros(demSub.shape, np.uint8)
	src_transform = newGt
	# make this not hard coded
	src_crs = pyproj.Proj(init='epsg:4326')
	dst_crs = pyproj.Proj(init='epsg:32606')
	ulx,uly=pyproj.transform(src_crs,dst_crs,lonMin,latMax)
	lrx,lry=pyproj.transform(src_crs,dst_crs,lonMax,latMin)
	src_crs = {'init': 'EPSG:4326'} # source coordinate system
	dst_crs = {'init': 'EPSG:32606'} # destination coordinate system
	dst_transform = rtransform.from_bounds(ulx,lry,lrx,uly,dCol,dRow)    
	...
	destination = np.squeeze(destination)
	
	# Make shapefile for swath profiling
	xInt,yInt = wgt.interpolateShapelyLineGeom(shapelyLineGeom,) # interpolate vertices using shapely
